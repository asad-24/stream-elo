...
type MediaOption = { id: string; title: string; mediaType: "image" | "video"; publicUrl: string; source: string };
...
export function PageSectionEditor({ section, media }: { section: Section; media: MediaOption[] }) {
  ...
  async function save(formData: FormData) {
    ...
    if (section.sectionType === "intro-gallery") {
      nextData.releaseLabel = String(formData.get("releaseLabel") ?? "");
      nextData.releaseTitle = String(formData.get("releaseTitle") ?? "");
    }
  ...
  return <form action={save} className="grid gap-5">
    ...
    {section.sectionType === "intro-gallery" ? <div className="grid gap-4 border border-papyrus/10 p-5 md:grid-cols-2"><label className="grid gap-2"><span className="label">Gallery label</span><input name="releaseLabel" defaultValue={String(data.releaseLabel ?? "New release")} className={field} /></label><label className="grid gap-2"><span className="label">Gallery title</span><input name="releaseTitle" defaultValue={String(data.releaseTitle ?? "The Iron River - Now streaming")} className={field} /></label></div> : null}
    ...
    <label className="grid gap-2"><span className="label">Section media</span><select multiple name="mediaIds" defaultValue={section.mediaIds} className={`${field} min-h-40 py-2`}>{media.map((item) => <option key={item.id} value={item.id}>{item.title} ({item.mediaType}, {item.source})</option>)}</select><span className="text-xs text-papyrus/45">Select images, R2 videos, or YouTube videos. Hold Ctrl/Cmd to select multiple items; their order controls the component.</span></label>
